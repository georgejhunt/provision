...
# create the languages database if it does not exist
if not os.path.isfile('{{ provision_assets_dir }}/languages.sqlite'):
   ...
   json_file = '{{ provision_assets_dir }}/lang_codes.json'
...
if not os.path.isfile( '{{ provision_assets_dir }}/catalog.sqlite' ):
   ...
   for zimkey in zimlist:
      try:
         ins = catalog.insert().values({
            ...
            u'iso3': data['zims'][zimkey][u'language'],
   ...
   for item in rachel_catalog:
      ...
      try:
         ins = catalog.insert().values({
            ...
            u'download_url': item[u'rsync_url'],
   ...
   #create table summary as select lan.iso2 as iso2,c.iso3 as iso3,engname,count(recno) as num from catalog as c left join lan.languages as lan  on c.iso3 = lan.iso3 group by iso3 order by num desc;
